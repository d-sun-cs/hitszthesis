...
\begin{table}[htbp]
    ...
    \begin{tabularx}{0.88\textwidth}{*{5}{>{\centering\arraybackslash}X}}
        \toprule
...
其中仅在个别两块丢失配置下略低于Cerasure（原因同上：两块丢失无法利用快路径），其余情形均全面领先，充分验证了优化方案在各类配置下的泛化性与有效性，也为后续系统实验提供了性能依据。
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{0.68\textwidth}{*{4}{>{\centering\arraybackslash}X}}
        \toprule
...
\FloatBarrier
\subsubsection{shift-XOR内部基线与消融实验}

由于当前移位异或纠删码领域尚缺少经过系统级优化、接口完整且易于复现实验的公开工程实现，无法直接找到与FastTT公平对比的成熟shift-XOR系统基线。
因此，本节将未经系统级优化的Two-tone直观实现（Two-tone naive）作为内部shift-XOR baseline，并将其放在表\ref{tab:ablation}第一行；该实现保留Two-tone基础编解码逻辑，但不包含CFWP、NMA、EPM、TPS等系统级优化。
在此基础上，本课题在$(8,3)$配置下进行消融实验，以同时评估各项优化的独立贡献和FastTT相对原始shift-XOR实现的优化收益；解码吞吐为所有丢失情况的平均值。
...
各项优化都对最终性能有清晰而显著的贡献，并共同构成FastTT性能提升的主要来源。
