...
本部分介绍中期阶段已完成的主要研究工作，包括移位异或纠删码原理分析、四项系统级优化方案设计与实现、Ceph系统集成与实验评估。
...
即每个数据块分别右移、不移或左移若干码字后异或得到校验块，图\ref{fig:tt-encode}展示了这一编码过程的直观示例。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{Example of Two-tone Encoding.pdf}
    \caption{Two-tone码编码示例}
    \label{fig:tt-encode}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{Example of Two-tone Decoding.pdf}
    \caption{Two-tone码解码示例}
    \label{fig:tt-decode}
\end{figure}
...
对上述算法进行分析可知，其性能受限于内存访问，主要存在三方面瓶颈：
...
针对上述瓶颈，本课题提出四项系统级优化技术并整合进\,FastTT\,实现。
